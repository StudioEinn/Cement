...
using Trite.Cement.Layouts.Collections;
using Trite.Cement.Layouts.Link;
using Trite.Cement.Layouts.Primitives;
using Trite.Cement.Layouts.References;
...
namespace Trite.Cement.Layouts
{
    public class PersistenceLayout<T> : ILayoutElement, IElementContainer
    {
        ...
        public PersistenceLayout<T> Collection<TElement>(string elementTag, Expression<Func<T, ICollection<TElement>>> property, Scope scope = Scope.None)
        {
            if (scope.HasFlag(Scope.Element) && scope.HasFlag(Scope.Collection))
            {
                this.Add(new DerivableCollectionWithDerivableChildrenElement<TElement>(elementTag, PropertyHelper.ToProperty(property)));
            }
            else if (scope.HasFlag(Scope.Element))
            {
                this.Add(new CollectionWithDerivableChildrenElement<TElement>(elementTag, PropertyHelper.ToProperty(property)));
            }
            else if (scope.HasFlag(Scope.Collection))
            {
        ...
        /// <param name="property">The property.</param>
        /// <param name="scope">The scope.</param>
        public PersistenceLayout<T> Collection<TElement>(string elementTag, string tag, Expression<Func<T, ICollection<TElement>>> property, Scope scope = Scope.None)
        {
            if (scope.HasFlag(Scope.Element) && scope.HasFlag(Scope.Collection))
            {
        ...
        /// <param name="setAction">The set action.</param>
        /// <param name="scope">The scope.</param>
        public PersistenceLayout<T> Collection<TElement>(string elementTag, string tag, Func<T, ICollection<TElement>> getAction, Action<T, ICollection<TElement>> setAction, Scope scope = Scope.None)
        {
            Func<object, object> getter = this.CreateGetAction(tag, getAction);
            Action<object, object> setter = this.CreateSetAction(tag, setAction);
