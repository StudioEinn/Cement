...
namespace Trite.Cement.Formats.Xml
{
    public class XmlWriter : IFormatWriter
    {
        ...
        {
            string[] array = value.Select(b => b.ToString()).ToArray();
            string data = string.Join(",", array);

            this._currentElement.Add(new XElement(tag, data));
        }
        /// <summary>
        ...
        public void Dispose()
        {
            ...
            using (XmlTextWriter writer = new XmlTextWriter(this.Stream, Encoding.Default))
            {
                writer.Formatting = Formatting.Indented;
                this._currentElement.Save(writer);
            }
        }
        #endregion
    }
